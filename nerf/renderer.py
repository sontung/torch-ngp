...
class NeRFRenderer(nn.Module):
    ...
    def run_depth_only(self, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # bg_color: [3] in range [0, 1]
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if num_steps not in self.num_steps_to_z_vals:
            z_vals_ori = torch.linspace(0.0, 1.0, num_steps, device=device).unsqueeze(0)  # [1, T]
            self.num_steps_to_z_vals[num_steps] = z_vals_ori
        else:
            z_vals_ori = self.num_steps_to_z_vals[num_steps]
        z_vals = z_vals_ori.expand((N, num_steps))  # [N, T]
        z_vals = nears + (fars - nears) * z_vals  # [N, T], in [nears, fars]
    ...
    def query_depth_only(self, rays_o, rays_d, staged=False, max_ray_batch=4096, **kwargs):
        B, N = rays_o.shape[:2]
        device = rays_o.device
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    tail = min(head + max_ray_batch, N)
                    results_ = self.run_depth_only(rays_o[b:b + 1, head:tail], rays_d[b:b + 1, head:tail], **kwargs)
                    depth[b:b + 1, head:tail] = results_['depth']
                    head += max_ray_batch
            results = {"depth": depth}

        else:
            results = self.run_depth_only(rays_o, rays_d, **kwargs)
